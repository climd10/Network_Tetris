...
namespace Tetris
{
    public partial class Form1 : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e) //게임을 실행하는 Main Timer 
        {
            ...
            this.Block_Falling();
            if (this.aitem == true)           //속도 조절 아이템이 사용 됐을 경우 250tick 동안 변경 된 속도를 유지
            {
        ...
        private void timer2_Tick(object sender, EventArgs e) // 남은 시간 표시 
        {
            ...
            else if (this.remain_minutes != 0 && this.remain_seconds == 60) // 0초일때 분과 초를 각각 1씩 깎는다 
            {
                this.remain_minutes--;
                this.remain_seconds--;
                if (remain_minutes == 1)                                    //1분 간격으로 속도 증가
                {
                    this.nspeed = 200;
                    this.timer1.Interval = this.nspeed;
                }
                else if (this.remain_minutes == 0)
                {
                    this.nspeed = 80;
                    this.timer1.Interval = this.nspeed;
                }
